<div>
    <x-header title="{{ __('users.edit.title', ['name' => $user->name]) }}" separator />
    ...
    <x-form wire:submit="save"> 
        ...
        <x-password label="{{ __('users.password') }}" wire:model="password" hint="{{ __('users.password_hint') }}" />

        <x-input label="{{ __('users.phone') }}" wire:model="phone" required />

        <x-input label="{{ __('users.company') }}" wire:model="company" required />

        <x-select label="{{ __('users.timezone') }}" wire:model="timezone" :options="array_map(fn ($item) => ['id' => $item, 'name' => $item], [null] + DateTimeZone::listIdentifiers())" />
